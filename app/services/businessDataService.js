...
const fetchBusinessDataBySlug = async (slug, query = null) => {
  console.log(
    "🔍 fetchBusinessDataBySlug called with slug:",
    slug,
    "query:",
    query
  );
  ...
  if (!query) {
    console.log(
      "📋 No query provided - fetching recent data from all collections"
    );
  ...
  const searchResults = await searchMultipleCollections(
    business._id,
    query,
    collections
  );
