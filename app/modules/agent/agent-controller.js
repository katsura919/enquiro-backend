  ...
  try {
    ...
    if (existingAgent) {
      return res.status(409).json({
        error: "Email address is already taken. Please use a different email.",
      });
    }
...
const restoreAgent = async (req, res) => {
  try {
    ...
    if (!agent)
      return res.status(404).json({ error: "Deleted agent not found" });
...
const getAgentsByBusiness = async (req, res) => {
  try {
    ...
    // Build query
    let query = Agent.find().where({ businessId: businessId });

    // Apply status filter
    if (status === "active") {
      query = query.where({ deletedAt: null });
    } else if (status === "inactive") {
      query = query.where({ deletedAt: { $ne: null } });
    }
    // if status is 'all', we don't add a deletedAt filter
    ...
    const inactiveAgentsCount = await Agent.countDocuments({
      businessId,
      deletedAt: { $ne: null },
    });
    const totalAgentsInBusiness = activeAgentsCount + inactiveAgentsCount;
    ...
    res.json({
      ...
      stats: {
        totalAgents: totalAgentsInBusiness,
...
const uploadProfilePicture = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to upload profile picture. Please try again." });
  }
};
...
const deleteProfilePicture = async (req, res) => {
  try {
    ...
    try {
      ...
      const urlParts = agent.profilePic.split("/");
      const publicIdWithExtension = urlParts[urlParts.length - 1];
      const publicId = `chat-uploads/profile-pictures/${
    ...
    res
      .status(500)
      .json({ error: "Failed to delete profile picture. Please try again." });
  }
};
